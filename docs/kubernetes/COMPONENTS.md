...
### Container
**What:** The running process. Here that is `uvicorn` serving the FastAPI app on
port 8000, from the image `tweakster24/insurance-premium-api:latest`.
...
immutable, with the model baked in, so there is no "load the right model" step at
...
- **LoadBalancer**: provisions a *dedicated cloud load balancer per Service*, so
...
`updateMode: "Off"`. This is **recommend-only, on purpose**: HPA and VPA must not
both act on CPU, or they fight each other. The VPA advises; a human applies the
...
questions, answered by three controllers that deliberately do not overlap.
...
**Here:** [`k8s/ingress/ingress.yaml`](../../k8s/ingress/ingress.yaml) sets
...
On EKS you do not run these; AWS does, for ~\$0.10/hr. You still need to
understand them to debug the cluster when it misbehaves.
