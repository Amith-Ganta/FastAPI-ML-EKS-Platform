...
This document explains every Kubernetes object and cluster component used by this
platform. For each one: **What it is**, **Why it exists** (the failure it
prevents), and **How it's used here** (grounded in the real manifests).

The ordering goes from the workload the user cares about, outward to the
...
**Why:** A container packages the app with its exact dependencies (scikit-learn
1.6, the pinned model) so it runs identically on a laptop, EC2, or EKS. It
...
is the boundary the scheduler places, the kubelet health-checks, and the Service
load-balances across.
**Here:** Each pod runs a single API container. Pods are **ephemeral** — a
reschedule gives a new IP — which is exactly why nothing addresses a pod
directly (see Service).

### ReplicaSet
**What:** A controller that keeps *N* identical pods running.
**Why:** Pods don't self-heal. If a pod (or its node) dies, something must
notice and recreate it. The ReplicaSet is that reconciler for a fixed count.
**Here:** You never write a ReplicaSet by hand — the Deployment creates and
manages one for you. Each rollout creates a **new** ReplicaSet (that's how
rollback works — see Deployment).

### Deployment
**What:** The declarative controller over ReplicaSets that manages rollouts and
rollbacks.
**Why:** You want to say "run this version, 2 replicas, roll updates one at a
time, and let me undo a bad release." A Deployment provides rolling updates,
revision history, and `kubectl rollout undo`.
**Here:** [`k8s/base/deployment.yaml`](../../k8s/base/deployment.yaml).
`replicas: 2` (availability floor), `maxUnavailable: 0 / maxSurge: 1` (never dip
below capacity during a rollout), three probes, and right-sized requests.

### Service
**What:** A stable virtual IP + DNS name that load-balances across the pods
matching its label selector.
**Why:** Pods are ephemeral and their IPs change. Clients need one address that
doesn't move. The Service is that stable front door; kube-proxy spreads traffic
across the currently-Ready pods only.
...
**Service types and why we chose ClusterIP + Ingress:**
- **ClusterIP** (default): internal-only stable IP. Used here for the API.
- **NodePort**: opens a high port on *every* node. Fine for a demo, ugly in
  prod (raw ports, no L7, no TLS).
...
### Namespace
**What:** A virtual cluster-within-the-cluster for grouping and isolating
objects.
**Why:** It scopes names, RBAC, quotas, and network policy. It's how you keep
`monitoring` separate from your app in `default`, and how you grant a team access
to only their slice.
...
**Why:** Configuration should live *outside* the image so the same immutable
image runs in dev/stage/prod with different settings. Secrets separate
credentials from code so they can be rotated and access-controlled.
**Here:** The app is configuration-light (model baked in), so there's no
hand-written ConfigMap yet; Grafana's admin password is the first thing that
...
**Why:** Traffic is spiky. A fixed replica count wastes money at idle or drops
requests at peak. The HPA adds/removes pods to track load.
**Here:** [`k8s/autoscaling/hpa.yaml`](../../k8s/autoscaling/hpa.yaml) — CPU
target 50% of request, min 2 / max 10. Slow scale-down (5-min window) prevents
flapping; fast scale-up absorbs bursts.

### VerticalPodAutoscaler (VPA)
**What:** Recommends (or sets) the *size* of each pod — its CPU/memory requests.
**Why:** You can't compute the right request from the node spec; it depends on
...
act on CPU or they fight. VPA advises; a human applies it at deploy time.

### Cluster Autoscaler
**What:** Scales the *number of nodes* (EC2 instances) in the node group.
**Why:** The HPA can request an 11th pod, but if every node is full it stays
`Pending`. The Cluster Autoscaler watches for unschedulable pods and adds a
node; it removes idle nodes to save cost.
**Here:**
[`k8s/autoscaling/cluster-autoscaler-values.yaml`](../../k8s/autoscaling/cluster-autoscaler-values.yaml)
— `least-waste` expander, 5-min scale-down, IRSA-scoped IAM. Matters because a
`t3.small` tops out near 11 pods (ENI/IP limit).

**The three-layer scaling story:** HPA (how many pods) → Cluster Autoscaler
(are there machines for them) → VPA (how big should each pod be). Three
different questions, three controllers, deliberately non-overlapping.
...
`group.name: insurance-platform` so one ALB is shared. It **reduces the number
of load balancers**, it doesn't replace the concept.
...
IPVS rules).
**Why:** A Service IP isn't a real interface — kube-proxy programs each node so
traffic to the ClusterIP is DNAT'd to a Ready pod.
**Here:** Runs as a DaemonSet on every node; you rarely touch it, but it's why
`ClusterIP` works.
...
On EKS you don't run these — AWS does, for ~\$0.10/hr — but you must understand
them to debug.

### kube-apiserver
**What:** The front door to the cluster; every `kubectl` call and every
controller talks to it.
**Why:** It's the single source of truth and the only writer to etcd. All
coordination flows through it.

### etcd
**What:** The consistent key-value store holding all cluster state.
**Why:** It's the database of record — every object you `kubectl get` lives here.
Losing it loses the cluster (which is why Velero backs up the *objects*).

### kube-scheduler
**What:** Decides which node each new pod runs on.
**Why:** Placement isn't trivial — it must respect requests, node capacity,
...
*requests* — which is exactly why right-sizing requests matters.
...
provides them (it keeps no history — that's Prometheus's job).
