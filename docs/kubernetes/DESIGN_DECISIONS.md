...
accepted. The point is not that these are the only right answers. Each one was a
*decision*, made against alternatives, not a default left unexamined.
...
- **Why:** A new pod takes ~15 to 30s to pull, start, and pass readiness. If you
...
- **Why:** `type: LoadBalancer` provisions a cloud LB *per Service*: three
  ...
  high ports on every node, with no L7, no TLS, operationally ugly.
...
- **Why:** Managed node groups handle AMI updates, draining, and ASG wiring, so
  there is less undifferentiated ops than self-managed nodes. `t3.small` is the cheapest
...
- **Why:** Prometheus is a scrape-and-store engine queried in-cluster, so there's
  no reason to expose it. Grafana is the human view, so it gets an address.
